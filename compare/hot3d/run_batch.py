...
Usage: run_batch.py <selection.json>
...
CFG = "configs/real_forehoi_icp_joint.yaml"
...
def main():
    ...
    for item in sel:
        ...
        run = f"{RC}/runs/hot3d_{cat}_{num}_icpj"
        try:
            ...
            if not os.path.exists(f"{run}/stage8_eval/pseudo_gt.npz"):
                ...
                sh([PY, "-m", "hoi_recon.cli", "--video", f"{inp}/rgb.mp4",
                    "--out", run, "--real", "--config", CFG, "--depth", "gt",
                    "--object-prompt", f"{px:.1f}", f"{py_:.1f}"],
                   cwd=RC, env=env)
            sh([PY, f"{HERE}/make_rc_vs_gt_overlay.py", inp, run,
                f"{HERE}/rc_vs_gt_{cat}_{num}.mp4"])
    ...
    json.dump(summary, open(f"{HERE}/batch_summary.json", "w"), indent=1)
    print("\n== batch summary ==")
    for s in summary:
        print(json.dumps(s))
